...
def generate_dynamic_test_matrix():
    ...
    5. "Performance_Data_Reconciliation" (性能压测与数据对账：【最高级指令】使用 Locust 模拟 100 个并发进行 10 秒压测。压测结束后，你必须通过 sqlalchemy 直连数据库执行 `SELECT COUNT(*) FROM eval_records`，对比数据库记录总数是否等于 Locust 成功发送的请求总数，实现“零丢单”自动化对账！)
    ...
    - "【Locust 语法铁律】：在编写 `self.client.post` 时，第一个参数必须是显式的字符串 URL 地址（如 '/api/v1/submit_eval'），绝对不能只传命名参数（keyword arguments）！"
    - "正确示例：self.client.post('/api/v1/submit_eval', files=..., name='/api/v1/submit_eval', catch_response=True)"
